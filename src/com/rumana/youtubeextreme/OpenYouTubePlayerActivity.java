...
public class OpenYouTubePlayerActivity extends Activity {
	...
	static {
    	...
    	System.loadLibrary("youtube-extreme");
    }
	//declare the jni functions
	private static native void startAccelerator(int port, int burst, int slp);
	...
	/** background task for running the youtube accelerator */
	protected AcceleratorTask mAcceleratorTask;
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		// get the proxy IP and port from the bundle passed to this intent
		Bundle bundle = this.getIntent().getExtras();
		String proxyIP = bundle.getString("ProxyIP");
		int proxyPort = bundle.getInt("ProxyPort");
		
		// TODO have to be done in an asynctask, find suitable place, and 
		// decide whether it has to be running or stopped
		// parameters for the C program is passed from here
		ArrayList<Integer> params = new ArrayList<Integer>();
		params.add(8888);
		params.add(1000);
		params.add(1);
		mAcceleratorTask = (AcceleratorTask) new AcceleratorTask().execute(params);
		//startAccelerator(8888, 1000, 1);
		...
		mQueryYouTubeTask = (QueryYouTubeTask) new QueryYouTubeTask(lYouTubeId).execute(this);
	}
	...
	private class AcceleratorTask extends AsyncTask<ArrayList<Integer>, Void, Void> {

		@Override
		protected Void doInBackground(ArrayList<Integer>... params) {
			int port = (int)params[0].get(0);
			int burst_size = (int)params[0].get(1);
			int delay = (int)params[0].get(2);
			startAccelerator(port, burst_size, delay);
			return null;
		}
	}
	 ...
	 * 
	 */
	private class QueryYouTubeTask extends AsyncTask<Context, ProgressUpdateInfo, Uri> {

		private boolean mShowedError = false;
		private YouTubeId utubeid;
		//private String proxyIp="N/A";
		//private int proxyPort=0;
		
		/**
		 * Contructor of asynctask class as a trick to pass multiple 
		 * different type parameters to the class. When calling the execute()
		 * we will pass the different parameters to the class itself, and then
		 * pass "this" to the execute call to call the constructor.
		 * 
		 * @param utubeid   ID of the video to play
		 * @param proxyIp   Proxy IP address to connect, N/A=no proxy
		 * @param proxyPort Proxy port to connect, 0=no proxy
		 */
		public QueryYouTubeTask(YouTubeId utubeid) {
		...
		@Override
		protected Uri doInBackground(Context... pParams) {
			String lUriStr = null;
			...
			try {
				...
				if(utubeid instanceof PlaylistId){
					publishProgress(new ProgressUpdateInfo(mMsgPlaylist));
					lYouTubeVideoId = YouTubeUtility.queryLatestPlaylistVideo((PlaylistId) utubeid);
				}
				
				else if(utubeid instanceof VideoId){
					lYouTubeVideoId = utubeid.getId();
				}
